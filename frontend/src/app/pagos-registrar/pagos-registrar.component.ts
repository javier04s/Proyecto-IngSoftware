...
export class PagosRegistrarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Obtener email del usuario autenticado y asignar al pago
  ...
  eliminarProducto(index: number) {
    ...
    this.aceptarProductos(); // recalcular monto
  }
    ...
    this.pagosService.obtenerPagoPorId(id).subscribe({
      next: (data: PagoResponse) => {
        this.pago = {
          ...
          productos: [] // ajustar si tienes productos asociados en edición
        };
      },
      error: () => {
